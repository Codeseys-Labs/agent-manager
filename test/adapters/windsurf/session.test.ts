...
/** Resolve the Windsurf User dir relative to a fake home, matching the reader. */
function userDirFor(home: string): string {
  if (process.platform === "darwin") {
    return join(home, "Library/Application Support/Windsurf/User");
  }
  if (process.platform === "win32") {
    return join(home, "AppData/Roaming/Windsurf/User");
  }
...
describe("Windsurf session reader", () => {
  ...
  test("supports alt `cascade.chatdata` key when primary chatdata absent", async () => {
    dir = await createTestDir("am-windsurf-session-");

    const userDir = userDirFor(dir.path);
    const wsDir = join(userDir, "workspaceStorage", "wsCascade");
    await mkdir(wsDir, { recursive: true });
    await Bun.write(
      join(wsDir, "workspace.json"),
      JSON.stringify({ folder: "file:///home/user/cascade" }),
    );
          ...
          tabs: [
            {
              ...
              bubbles: [{ type: "user", text: "cascade hello" }],
            },
          ],
        }),
      );
    ...
    expect(sessions).toHaveLength(1);
    expect(sessions[0].id).toBe("wsCascade:cascade-tab");
    expect(sessions[0].project).toBe("/home/user/cascade");
  });
