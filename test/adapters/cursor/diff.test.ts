...
describe("cursor diffConfig()", () => {
  ...
  test("in-sync when native matches resolved", async () => {
    dir = await createTestDir("am-cursor-diff-");
    await dir.write(
      ".cursor/mcp.json",
      JSON.stringify({
        mcpServers: {
  ...
  test("merges global and project servers for diff", async () => {
    ...
    await dir.write(
      ...
      JSON.stringify({
        mcpServers: {
          "db-mcp": { command: "npx", args: ["-y", "db-mcp-server"] },
        },
      }),
    );
    ...
    const result = diffConfig(cfg, { projectPath: projectDir }, dir.path);
    expect(result.status).toBe("in-sync");
  });
});
