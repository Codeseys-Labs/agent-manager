...
describe("continue diffConfig()", () => {
  ...
  test("in-sync when native matches resolved", async () => {
    dir = await createTestDir("am-ct-diff-");
    await dir.write(
      ".continue/config.json",
      JSON.stringify({
        mcpServers: [{ name: "fetch", command: "uvx", args: ["mcp-server-fetch"] }],
      }),
    );
  ...
  test("detects modified server fields", async () => {
    ...
    await dir.write(
      ".continue/config.json",
      JSON.stringify({
        mcpServers: [{ name: "tavily", command: "bunx", args: ["tavily-mcp@0.9.0"] }],
      }),
    );
