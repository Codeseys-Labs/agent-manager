...
describe("marketplace command", () => {
  ...
  test("marketplace command has correct meta", async () => {
    const meta = await resolveMeta(marketplaceCommand);
    expect(meta?.name).toBe("marketplace");
    expect(meta?.description).toContain("marketplace");
  });

  test("marketplace command has all subcommands", async () => {
    const subCmds = await resolveSubCommands(marketplaceCommand);
  ...
  test("add subcommand requires url positional", async () => {
    const subCmds = await resolveSubCommands(marketplaceCommand);
    const addCmd = subCmds!.add as any;
    expect(addCmd.args?.url?.type).toBe("positional");
    expect(addCmd.args?.url?.required).toBe(true);
  });

  test("install subcommand requires plugin positional", async () => {
    const subCmds = await resolveSubCommands(marketplaceCommand);
    const installCmd = subCmds!.install as any;
    expect(installCmd.args?.plugin?.type).toBe("positional");
    expect(installCmd.args?.plugin?.required).toBe(true);
  });

  test("search subcommand requires query positional", async () => {
    const subCmds = await resolveSubCommands(marketplaceCommand);
    const searchCmd = subCmds!.search as any;
    expect(searchCmd.args?.query?.type).toBe("positional");
    expect(searchCmd.args?.query?.required).toBe(true);
  });

  test("remove subcommand requires name positional", async () => {
    const subCmds = await resolveSubCommands(marketplaceCommand);
    const removeCmd = subCmds!.remove as any;
    expect(removeCmd.args?.name?.type).toBe("positional");
    expect(removeCmd.args?.name?.required).toBe(true);
  });

  test("all subcommands support --json flag", async () => {
    const subCmds = (await resolveSubCommands(marketplaceCommand))!;
